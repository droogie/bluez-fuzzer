...
static gboolean opt_fuzz = FALSE;
...
static void primary_all_cb(uint8_t status, GSList *services, void *user_data)
{
	...
	for (l = services; l; l = l->next) {
		...
		g_print("hmmm? attr handle = 0x%04x, end grp handle = 0x%04x "
...
static gboolean fuzz_gatt(gpointer user_data)
{
	GAttrib *attrib = user_data;
...
static GOptionEntry gatt_options[] = {
	...
	{ "fuzz", 0, 0, G_OPTION_ARG_NONE, &opt_fuzz, 
		"Fuzz GATT Services", NULL },
	{ NULL },
};
