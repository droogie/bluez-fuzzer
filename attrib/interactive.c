...
static void disconnect_io()
{
	...
	// https://www.spinics.net/lists/linux-bluetooth/msg67617.html
...
static void fuzz_all_cb(uint8_t status, GSList *services, void *user_data)
{
	GSList *l;
	uint8_t fuzz_data[23] = {0};
	...
	int service_cnt = g_slist_length(services);
	printf("total services discovered: %d\n", service_cnt);

	while (1) {
	//for (int i=0; i < 4096*100; i++) {
		struct gatt_primary *prim = g_slist_nth_data(services, rand() % service_cnt);
		uint16_t handle = (uint16_t) rand_range(prim->range.start, prim->range.end);
		
		//0006, 0025, 002a, 0032, 0035, 003b
		//uint16_t handles[] = {0x0006, 0x0025, 0x002a, 0x0032, 0x0035, 0x003b};
		//uint16_t handle = handles[rand() % 6];

		int plen = rand() % 23; // up to 22 bytes
		for (int i=0; i < plen; i++) {
			fuzz_data[i] = (uint8_t) rand();
		}
		printf("fuzzing handle: 0x%04x len: 0x%02x\n", handle, plen);
		hexdump(fuzz_data, plen);

		if (rand() % 2 == 0) {
			gatt_write_char(attrib, handle, fuzz_data, plen, char_write_req_cb, NULL);
		} else {
			if (!gatt_write_cmd(attrib, handle, fuzz_data, plen, NULL, NULL)) {
...
static void cmd_fuzz(int argcp, char **argvp)
{
	bt_uuid_t uuid;

	printf("Fuzzer starting!\n");
	init_rand(0); // add seed args later...
	...
	if (argcp == 1) {
		gatt_discover_primary(attrib, NULL, fuzz_all_cb, NULL);
		return;
	}
...
static struct {
	...
	{ "fuzz",		cmd_fuzz,	"[start hnd] [end hnd]",
